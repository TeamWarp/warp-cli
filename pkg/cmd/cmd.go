	...
	Command = &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&requestflag.Flag[string]{
				Name:    "api-key",
				Sources: cli.EnvVars("WARP_API_KEY"),
			},
		},
		Commands: []*cli.Command{
			...
			},
			{
				...
				Commands: []*cli.Command{
					...
					&workersList,
					&workersDelete,
					&workersCreateContractor,
					&workersCreateEmployee,
					&workersInvite,
				},
			},
			{
			...
			},
			{
				...
				UsageText:       "warp-hr @manpages [-o warp-hr.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "warp-hr.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "warp-hr.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
